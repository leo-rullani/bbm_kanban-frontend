...
/* ------------------------------------------------------------------
   Graphics board – Extras: Button "Kits" + Overlays (Grid → Team)
   (mit Server-Persistenz über /api/kv/bbm.kitPairs.v2/)
------------------------------------------------------------------ */
(function () {
  const PAIRS_KEY = 'bbm.kitPairs.v2'; // lokaler Cache
  const KV_KEY    = PAIRS_KEY;         // Server-Key identisch
  const ICON_BASE = '../../assets/icons/';
  const LEAGUE_LOGO = ICON_BASE + 'brack_super_league.svg.png';
  ...
  // Offizielle Vereinsfarben (Primär)
  const CLUB_COLORS = {
    fcb:      '#ff0000', fcluzern: '#1e467d', gcz: '#005791', ls: '#00248b',
    fclugano: '#000000', sfc: '#66202f', fcwin: '#d21217', fcz: '#002855',
    fcsg:     '#007c3e', fcthun: '#ed272d', fcsion: '#ed2424', yb: '#ffdd00',
  };
  const HOVER_COLORS = { ...CLUB_COLORS };
  ...
  // Lädt Wert vom Server und aktualisiert localStorage‑Cache
  async function syncPairsFromServer() {
    try {
      const resp = await fetch(API_KV_URL(KV_KEY), { credentials: 'include' });
      if (!resp.ok) return loadPairs(); // Fallback belassen
      const data = await resp.json();
      if (data && data.value && typeof data.value === 'object') {
        localStorage.setItem(PAIRS_KEY, JSON.stringify(data.value));
        return data.value;
      }
    } catch (_) {}
    return loadPairs();
  ...
  // Debounced PUT → Server, + sofort in localStorage cachen
  function kvSaveDebounced(key, value, wait = 400) {
    ...
    _kvTimers[key] = window.setTimeout(async () => {
      try {
        ...
        // still cached locally; next edit will retry
      }
    }, wait);
  }
  ...
  function injectKitsStyle() {
    ...
    const css = `
      ...
      .gfx-kits-close{margin-left:auto;border:0;background:#1f2937;color:#e5e7eb;border-radius:8px;padding:6px 10px;cursor:pointer}
      .gfx-kits-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}
      .gfx-kits-tile{background:#0b1020;border:1px solid #1f2a44;border-radius:10px;padding:14px;text-align:center;display:grid;gap:8px;cursor:pointer;transition:transform .15s ease, background-color .15s ease, color .15s ease, border-color .15s ease, box-shadow .15s ease}
      .gfx-kits-crest{height:64px;width:64px;object-fit:contain;display:block;margin:0 auto;transition:transform .15s ease, filter .15s ease}
      .gfx-kits-name{font-size:13px;opacity:.9}
      .gfx-kits-tile:hover{transform:translateY(-2px) scale(1.02); background:var(--hover-bg, #101a33); color:var(--hover-fg, #e5e7eb); border-color:rgba(255,255,255,.25); box-shadow:0 10px 28px rgba(0,0,0,.35)}
      .gfx-kits-tile:hover .gfx-kits-crest{transform:scale(1.12); filter:drop-shadow(0 6px 10px rgba(0,0,0,.35))}
      .gfx-kits-table{width:100%;border-collapse:collapse}
      .gfx-kits-table th,.gfx-kits-table td{border:1px solid #233152;padding:8px 10px;text-align:left;font-size:13px}
      .gfx-kits-table th{background:#111a2f}
      .gfx-kits-hex{width:120px;text-align:center;font-family:monospace;border:1px solid #2b3852;border-radius:6px;padding:6px;background:#0a0f1e;color:#e5e7eb}
      .gfx-kits-font{width:92px;text-align:center;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;border:1px solid #2b3852;border-radius:20px;padding:6px 10px;cursor:pointer;background:#111a2f;color:#e5e7eb;transition:transform .12s ease, filter .12s ease, background-color .12s ease, color .12s ease}
      .gfx-kits-font:hover{transform:translateY(-1px); filter:brightness(1.05)}
      .gfx-kits-teamhead{display:flex;align-items:center;gap:10px;margin:4px 0 10px}
      .gfx-kits-teamhead img{height:28px;width:28px;object-fit:contain}
      .gfx-kits-back{margin-left:auto;border:0;background:#1f2937;color:#e5e7eb;border-radius:8px;padding:6px 10px;cursor:pointer}
  ...
  function savePairs(obj) {
    // lokal sofort und dann debounced an den Server
    kvSaveDebounced(KV_KEY, obj);
  }
  function pairKey(homeId, awayId) { return `${homeId}|${awayId}`; }
  ...
  /* ===================== Team‑Ansicht ===================== */
  async function showTeam(homeId) {
    // Vor dem Rendern mit Server synchronisieren (falls möglich)
    await syncPairsFromServer();
    ...
    CLUBS.filter(c => c.id !== homeId).forEach(awayClub => {
      ...
      // Schriftfarbe-Regel: per Default Weiß, YB Schwarz
      setFontPill(homePill, (homeId === 'yb') ? '#000000' : '##FFFFFF'.slice(1), true);
      setFontPill(awayPill, (awayClub.id === 'yb') ? '#000000' : '##FFFFFF'.slice(1), true);
